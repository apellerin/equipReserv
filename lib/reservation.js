...
var mailout = require('./mailout.js');
...
exports.updateReservationStatus = function (reservation_id, reserv_status, cb) {
    async.waterfall([
        ...
        function (connection, callback) {
            ...
            connection.query(sql, function (err, results) {
                sql_pool.release(connection);
                setTimeout(sendStatusChangeEmail(reservation_id, reserv_status), 3000);
                cb(results)
            });
        }], function (err) {
               if(err) throw new Error(err);
...
exports.getAvailableEquipment = function (length, page, filter, start, end, cb) {
    ...
    async.waterfall([
        ...
        function (connection, callback) {
            var sql = 'SELECT \
                            ...
                            LEFT JOIN\
                                    (\
                                        ...
                                        WHERE \
                                            ((res.reserv_start_date BETWEEN ? AND ?) OR \
                                             res.reserv_start_date < ? AND \
                                             (res.reserv_end_date > ? OR res.reserv_end_date BETWEEN ? AND ? )) \
                                        AND res.reserv_status IN(2,5,7) \
                                        UNION ALL SELECT inventory_id FROM reserv_cart rc \
                                        WHERE \
                                        ((rc.startdate BETWEEN ? AND ?) OR \
                                             rc.startdate < ? AND \
                                             (rc.enddate > ? OR rc.enddate BETWEEN ? AND ? )) \
				                )resitems \
            ...
            var inserts = [start, end, start, start, start, end, start, end, start, start, start, end, (length * page), length];
            sql = mysql.format(sql, inserts);
            connection.query(sql, function (err, results) {
                sql_pool.release(connection);
                callback(err, results);
        ...
        //GET SQL Connection
         ...
         function (connection, callback) {
             ...
             sql = 'SELECT \
                         ...
                         equipment.equip_id = ? \
                            LIMIT 1';
             inserts = [start, end, start, start, start, end, start, end, start, start, start, end, item];
             sql = mysql.format(sql, inserts);
             connection.query(sql, function (err, results) {
                if(err) throw new Error(err);
                sql_pool.release(connection);
...
exports.finalizeCart = function (user, reservation_id, cb) {
    ...
    exports.getCartItemsByUser(user, function (result) {
        ...
        exports.clearUserCart(user, function (result) {
            setTimeout(sendReservationConfirmation(user, reservation_id), 3000);
            cb(result);
        });
    });
};
    ...
    async.waterfall([
        ...
        function (connection, callback) {
            var sql = "SELECT DISTINCT reservation_id, last_name, first_name, reserv_start_date, reserv_end_date, \
                        ...
                        OR inventory_id like '%" + filter + "%' OR make like '%" + filter + "%' OR model like '%" + filter + "%' \
    ...
    async.waterfall([
        ...
        function (connection, callback) {
            var sql = "SELECT DISTINCT reservation_id, last_name, first_name, reserv_start_date, reserv_end_date, \
                        status_description, phone, email, status_id FROM vw_allresdata \
                        WHERE (reservation_id like '%" + filter + "%' OR last_name like '%" + filter + "%' \
                        OR first_name like '%" + filter + "%' OR reserv_start_date like '%" + filter.replace("/","-") + "%' \
                        OR reserv_end_date like '%" + filter.replace("/","-") + "%' OR status_description like '%" + filter + "%' \
                        OR phone like '%" + filter + "%' OR email like '%" + filter + "%' \
                        OR inventory_id like '%" + filter + "%' OR make like '%" + filter + "%' OR model like '%" + filter + "%') \
    ...
    async.waterfall([
        ...
        function (connection, callback) {
            var sql = "SELECT DISTINCT reservation_id, last_name, first_name, reserv_start_date, reserv_end_date, \
                        ...
                        OR phone like '%" + filter + "%' OR email like '%" + filter + "%' \
                        OR inventory_id like '%" + filter + "%' OR make like '%" + filter + "%' OR model like '%" + filter + "%') \
                        AND status_id IN(2,5,7) \
                        AND (CAST(reserv_end_date AS DATE) = CAST(NOW() AS DATE) OR CAST(reserv_start_date AS DATE) = CAST(NOW() AS DATE)) \
                        ORDER BY reserv_start_date ASC LIMIT ?,?";
         ...
         function (connection, callback) {
             ...
             sql = 'SELECT \
                         ...
                         equipment.equip_id = ? AND eqdata.inventory_id <> ? \
                            LIMIT 1';
             inserts = [start, end, start, start, start, end, start, end, start, start, start, end, item, oldid];
             sql = mysql.format(sql, inserts);
             connection.query(sql, function (err, results) {
                if(err) throw new Error(err);
                sql_pool.release(connection);
...
sendReservationConfirmation = function (user, reservation_id) {
    async.waterfall([
        //Acquire SQL connection from pool
        function (callback) {
            sql_pool.acquire(function (err, connection) {
        ...
        function (connection, callback) {
            var sql = "SELECT DISTINCT reservation_id, last_name, first_name, reserv_start_date, reserv_end_date, \
                        status_description, phone, email FROM vw_allresdata \
                        WHERE reservation_id = ?";
            ...
            connection.query(sql, function (err, results) {
                sql_pool.release(connection);
                try {
                    results[0].reserv_start_date = moment(results[0].reserv_start_date).format('LLL');
                    results[0].reserv_end_date = moment(results[0].reserv_end_date).format('LLL');
                    callback(err, results);
                } catch(e) {
        ...
        function (results, callback) {
            var dataobj = {
            content : mail_props.reservation_confirmation_email.text,
            heading : 'Confirmation',
            link: mail_props.reservation_confirmation_email.link,
            contactemail: local.config.admin_contact.email,
            reservationinfo: results[0]
            };

            var filepath = 'views/email/confirmationemail.jade';
            var email = results[0].email;

            mailout.compileHTMLEmail(filepath, dataobj, function (html) {
                mailout.sendEmail(null, html, email, mail_props.reservation_confirmation_email.subject);
            });
...
sendStatusChangeEmail = function (reservation_id, reserv_status) {
    async.waterfall([
        //Acquire SQL connection from pool
        function (callback) {
            sql_pool.acquire(function (err, connection) {
        ...
        function (results, callback) {
            var dataobj = {
            content : mail_props.reservation_statuschange_email.text,
            heading : 'Reservation Status Change',
            link: mail_props.reservation_statuschange_email.link,
            contactemail: local.config.admin_contact.email,
            reservationinfo: results[0]
            };

            var filepath = 'views/email/statuschangeemail.jade';
            var email = results[0].email;

            mailout.compileHTMLEmail(filepath, dataobj, function (html) {
                mailout.sendEmail(null, html, email, mail_props.reservation_statuschange_email.subject);
            });
...
exports.getContractData = function (reservation_id, cb) {
async.waterfall([
        //Acquire SQL connection from pool
        function (callback) {
            sql_pool.acquire(function (err, connection) {
        ...
        //Prepare and execute SQL
        function (connection, callback) {
            var sql = "SELECT * FROM vw_allresdata \
                        WHERE reservation_id = ?";
