...
exports.getAvailableEquipment = function (start, end, type, cb) {
    ...
    async.waterfall([
        ...
        //Prepare and execute SQL
        function (connection, callback) {
            var sql = 'SELECT DISTINCT equipment.* \
                        FROM equipment \
                        INNER JOIN ( \
                                ...
                                LEFT JOIN \
                                        ...
                                        WHERE \
                                            (res.reserv_start_date \
                                                BETWEEN ? AND ? \
                                                or res.reserv_end_date > ?) \
                                        AND res.reserv_status <> 3 \
                                        )resitems \
                                ON e.inventory_id = resitems.inventory_id \
                                WHERE resitems.inventory_id IS NULL) eqdata \
                                ON equipment.equip_id = eqdata.equip_id \
                                WHERE equipment.type_id = ?';
            
            ;
            var inserts = [start, end, start, type];
            sql = mysql.format(sql, inserts);
            connection.query(sql, function (err, results) {
                sql_pool.release(connection);
                callback(err, results);
...
exports.reserveItem = function (reservation_id, inventory_id, cb) {
    async.waterfall([
      //Acquire SQL connection from pool
       function (callback) {
           sql_pool.acquire(function (err, connection) {
