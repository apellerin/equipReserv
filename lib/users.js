...
invalidCredentials = function(req, res){
    res.render('./users/login',{message: messages.invalid_credentials, user: req.session.thisUser});
}
//USER REGISTRATION
               ...
               async.waterfall([
                   //Acquire SQL connection from pool
                    ...
                    function(connection, callback){
                        ...
                        var inserts = [req.body.user_name, req.body.password, req.body.first_name, req.body.last_name,
...
exports.update = function(req, res){
    getByHash(req.session.thisUser.hash, function(user){
        thisUser = user;
        if(thisUser.validatePassword(req.body.password)){
            if(!req.body.user_name || !req.body.password || !req.body.first_name || !req.body.last_name || !req.body.email ){
                   res.render('./users/account',{user: req.session.thisUser, message: messages.invaliduserupdate});
                   return;
            }
            else { 
                getByUsername(req.body.user_name, function(user){
                    if(user && req.body.user_name != req.session.thisUser.user_name){
                        res.render('./users/account',{user: req.session.thisUser, message: messages.username_exists});        
                    }
                    else {
                        getByEmail(req.body.email, function(user){
                            if(user && req.body.email != req.session.thisUser.email){
                                res.render('./users/account',{user: req.session.thisUser, message: messages.email_exists});    
                            }
                            else {
...
exports.changePassword = function(req, res){
    getByHash(req.session.thisUser.hash, function(user){
        ...
        if(! thisUser.validatePassword(req.body.oldpassword)){
            res.render('./users/changepass',{user: req.session.thisUser, message: messages.invalidpassword});  
        }
        else {
