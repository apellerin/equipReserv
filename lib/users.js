...
invalidCredentials = function(req, res){
    req.session.session_message = 'Invalid credentials.  Please log in or register';
    res.render('login',{title : 'Login', message: req.session.session_message});
}
//USER REGISTRATION
exports.register = function(req, res) {
    ...
    if(!req.body.username || !req.body.password || !req.body.first_name || !req.body.last_name || !req.body.email ){
           req.session.session_message = 'Registration form missing data.  Please correct and resubmit.';
           res.render('register',{title : 'Register', message: req.session.session_message});
           }  else {
               async.waterfall([
                   //Acquire SQL connection from pool
                    function(callback){
                    ...
                    function(connection, callback){
                        ...
                        var activation_hash = bcrypt.hashSync(req.body.username, 8);
                    ...
                    function(results, activation_hash, callback){
                        ...
                        req.session.session_message = 'An error occurred.  Please retry your request later.';
                        try {
                        res.redirect(req.headers['referer'],{message: session_message}); 
                        }catch(e) {
