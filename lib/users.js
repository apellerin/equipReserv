                            ...
                            if(!thisUser.validatePassword(req.body.password)) {
                                ...
                                if(thisUser.activated == 1) {
                                    ...
                                    res.render('index',{title: 'Welcome'});
                                        ...
                                        res.render('./users/login',{notactivated: thisUser.response_obj()});
                                            }
                                }
                        }
                    }
...
invalidCredentials = function(req, res){
    res.render('./users/login',{invalidcredentials: messages.invalid_credentials});
}
//USER REGISTRATION
exports.register = function(req, res) {
    ...
    if(!req.body.username || !req.body.password || !req.body.first_name || !req.body.last_name || !req.body.email ){
           res.render('./users/register',{invalidreg: messages.incomplete_reg});
           }  else {
               async.waterfall([
                   //Acquire SQL connection from pool
                    function(callback){
                    ...
                    function(results, hash, callback){
                        if(!results) {
                            res.render('./users/register',{invalidreg: messages.user_exists});
                        } else {
                            sendActivationLink(hash, req.body.email);
                            res.render('./users/login',{reg_success: messages.reg_success});
                        }
                    }
                ], function (err, result) {
                        try {
        ...
        async.waterfall([
                   //Acquire SQL connection from pool
                    ...
                    function(connection, callback){
                        ...
                        connection.query(sql, function(err, results) {
                              sql_pool.release(connection);
                              res.render('./users/login',{activatesuccess: messages.post_activate});  
                            });   
                     }
                ], function (err, result) {
                        res.send(messages.generic_error);
                   ...
                   //Acquire SQL connection from pool
                    ...
                    function(result){
                        getByUsername(newUser.user_name, function(user){
                            ...
                            res.render('./users/account',req.session.thisUser);
                        });
                    }
                ], function (err, result) {
                        res.send(messages.generic_error);
...
exports.recoverPassword = function(req, res){
    ...
    async.waterfall([
                   //Acquire SQL connection from pool
                    ...
                    function(results, callback) {
                        if(results.length == 0) {
                            res.render('./users/login',{invalidcredentials: messages.no_matching_account});
                        } else {
                    ...
                    function(hash){
                        ...
                        res.render('./users/login',{email: email, resetpassword: true});
                    }
                ], function (err, result) {
                        res.send(messages.generic_error);
