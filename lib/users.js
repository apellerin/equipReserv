...
var local = require('../local.config.js');
    ...
    title = local.config.hostconfig.app_title;
...
exports.register = function(req, res) {
    ...
    if(!req.body.user_name || !req.body.password || !req.body.first_name || !req.body.last_name || !req.body.email ){
           res.render('./users/register',{message: messages.incomplete_reg, title: title});
           }  
        else {
               async.waterfall([
                   //Acquire SQL connection from pool
                    ...
                    function(results, hash, callback){
                        if(!results) {
                            res.render('./users/register',{message: messages.user_exists, title: title});
                        } else {
                            sendActivationLink(hash, req.body.email);
                            res.render('./users/login',{message: messages.reg_success, title: title});
                        }
                    }
               ], function (err, result) {
                        try {
                        res.redirect(req.headers['referer'],{message: messages.generic_error, title: title}); 
                        }catch(e) {
                        ...
                        if(thisUser === null){
                            ...
                            if(!thisUser.validatePassword(req.body.password)) {
                                ...
                                if(thisUser.activated == 1) {
                                    ...
                                    res.render('./users/login',{notactivated: thisUser.response_obj(), message: messages.notactivated, title: title});
                                        }
                                }
                        }
                    }
...
invalidCredentials = function(req, res){
    res.render('./users/login',{message: messages.invalid_credentials, user: null, title: title});
}
...
var sendActivationLink = function(hash, email){
        var dataobj = {
            content : mail_props.user_activation_email.text,
            heading : 'Welcome !',
            link: mail_props.user_activation_email.link + hash,
            contactemail: local.config.admin_contact.email 
        };

        var filepath = 'views/email/activationemail.jade';

        mailout.compileHTMLEmail(filepath, dataobj, function (html) {
            mailout.sendEmail(null, html, email, mail_props.user_activation_email.subject);
        });
}
        ...
        async.waterfall([
                   //Acquire SQL connection from pool
                    ...
                    function(connection, callback){
                        ...
                        connection.query(sql, function(err, results) {
                              sql_pool.release(connection);
                              res.render('./users/login',{message: messages.post_activate, title: title});  
                            });   
                     }
                ], function (err, result) {
                        res.send(messages.generic_error);
...
exports.recoverPassword = function(req, res){
    ...
    async.waterfall([
                   //Acquire SQL connection from pool
                    ...
                    function(results, callback) {
                        if(results.length == 0) {
                            res.render('./users/login',{message: messages.no_matching_account, title: title});
                        } else {
                    ...
                    function(hash){
                        ...
                        var dataobj = {
                            content : mail_props.reset_password_email.text,
                            heading : 'Reset Password:',
                            link: mail_props.reset_password_email.link + hash,
                            contactemail: local.config.admin_contact.email 
                        };
                        ...
                        res.render('./users/login',{email: email, resetpassword: true, message: messages.resetpassword, title: title});
    ...
    exports.getByHash(req.session.thisUser.hash, function(user){
        ...
        if(thisUser.validatePassword(req.body.password)){
            if(!req.body.user_name || !req.body.password || !req.body.first_name || !req.body.last_name || !req.body.email ){
                   ...
                   res.render('./users/account',{user: req.body, message: messages.invaliduserupdate, title: title});
                   return;
            }
            else { 
                getByUsername(req.body.user_name, function(user){
                    if(user && (req.body.user_name != req.session.thisUser.user_name)){
                        req.body.user_level = req.session.thisUser.user_level;
                        res.render('./users/account',{user: req.body, message: messages.username_exists, title: title});        
                    }
                    else {
                        getByEmail(req.body.email, function(user){
                            if(user && (req.body.email != req.session.thisUser.email)){
                                req.body.user_level = req.session.thisUser.user_level;
                                res.render('./users/account',{user: req.body, message: messages.email_exists, title: title});    
                            }
                            else {
            ...
            req.body.user_level = req.session.thisUser.user_level;
            res.render('./users/account',{user: req.body, message: messages.invalidpassword, title: title});  
        }
    });
}
        ...
        async.waterfall([
            ...
            function(result){
                getByUsername(newUser.user_name, function(user){
                    ...
                    res.render('./users/account',{user: req.session.thisUser, message: messages.accountupdated, title: title});
                });
            }
        ], function (err, result) {  
            res.render('index',{message: {message: err, messagetype: 'error',messageheader: 'OOOOPS!  '}, user: req.session.thisUser, title: title});
...
exports.changePassword = function(req, res){
    exports.getByHash(req.session.thisUser.hash, function(user){
        ...
        if(! thisUser.validatePassword(req.body.oldpassword)){
            res.render('./users/account',{user: req.session.thisUser, message: messages.invalidpassword, title: title});  
        }
        else {
...
updatePassword = function(req, res){
    ...
    if(oldpass == newpass){
        res.render('./users/account',{user: req.session.thisUser, message: messages.noupdate, title: title});
    }
        else { 
            ...
            async.waterfall([
                ...
                function(result){
                    getByUsername(req.session.thisUser.user_name, function(user){
                        req.session.thisUser = user;
                        res.render('./users/account',{user: user ,message: messages.accountupdated, title: title});
                    });
                }
            ], function (err, result) {  
                res.render('./users/account',{message: {message: err, messagetype: 'error',messageheader: 'OOOOPS!  '}, user: req.session.thisUser, title: title});
...
exports.adminUpdate = function (req, cb) {
        ...
        inserts = [req.body.user_name, req.body.first_name, req.body.last_name,
                    req.body.email, req.body.phone, parseInt(req.body.user_level) || 0,
                    parseInt(req.body.activated) || 0, req.body.user_name];
...
exports.sendMessage = function(message) {
    var dataobj = {
            ...
            email: local.config.admin_contact.email,
            contactemail: local.config.admin_contact.email
            };

            var filepath = 'views/email/customercontactemail.jade';

            mailout.compileHTMLEmail(filepath, dataobj, function (html) {
                mailout.sendEmail(null, html, dataobj.email, 'Customer Contact');
            });
}