...
sendActivationLink = function(hash, email){
        ...
        var text = local.config.emails.user_activation_email.text + local.config.emails.user_activation_email.link + hash;
        var to = email;
...
exports.recoverPassword = function(req, res){
    ...
    async.waterfall([
                   //Acquire SQL connection from pool
                    ...
                    function(results, callback) {
                        if(results.length == 0) {
                            res.render('forgotpassword',{message: 'The email supplied does not match a user account.'});
                        } else {
                    ...
                    function(hash){
                        ...
                        var link = 'http://localhost:3000/resetpassword/?id='+ hash;
                        var text = 'Please follow the link to reset your password. ' + link;
                        var to = email;
